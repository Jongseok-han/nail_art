...
export default function AetherHero({
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className={`relative z-[2] h-full flex items-center ${justifyClass} p-[min(6vw,64px)]`}
        style={{ color: textColor }}
      >
        <div
          className={`w-full ${textAlignClass} ${mxClass}`}
          style={{ maxWidth }}
        >
          <h1 className="m-0 font-bold [font-family:var(--font-indie-flower),cursive] [font-size:clamp(2.2rem,4vw,2.5rem)] [line-height:1.04] tracking-[-0.02em] [text-shadow:0_6px_36px_rgba(0,0,0,0.45)]">
            {title}
          </h1>

          {subtitle && (
            <p className={`mt-4 [font-size:clamp(0.8rem,2vw,1rem)] [line-height:1.5] opacity-80 [text-shadow:0_4px_24px_rgba(0,0,0,0.35)] max-w-[900px] ${mxClass}`}>
              {subtitle}
            </p>
          )}
          ...
          {ctaLabel && (
            <div className="inline-flex gap-3 mt-8 flex-wrap">
              <a
                href={ctaHref}
                className="px-[18px] py-3 rounded-xl no-underline font-semibold [background:linear-gradient(180deg,rgba(255,255,255,.18),rgba(255,255,255,.06))] shadow-[inset_0_0_0_1px_rgba(255,255,255,.28),0_10px_30px_rgba(0,0,0,.2)] backdrop-blur-[6px] backdrop-saturate-[120%]"
                style={{ color: textColor }}
              >
                {ctaLabel}
              </a>
